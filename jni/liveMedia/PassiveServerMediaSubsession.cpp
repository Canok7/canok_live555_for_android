...
void PassiveServerMediaSubsession::startStream(unsigned clientSessionId,
  ...
  if (fRTCPInstance != NULL) {
    ...
    RTCPSourceRecord* source = (RTCPSourceRecord*)(fClientRTCPSourceRecords->Lookup(reinterpret_cast<const char*>(clientSessionId)));
    if (source != NULL) {
...
void PassiveServerMediaSubsession::deleteStream(unsigned clientSessionId, void*& /*streamToken*/) {
  ...
  if (source != NULL) {
    ...
    fClientRTCPSourceRecords->Remove(reinterpret_cast<const char*>(clientSessionId));
    delete source;
  }
}
